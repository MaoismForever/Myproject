...
public class HttpApiInvoker implements InvocationHandler {
    ...
    private static final Pattern VARIABLE_PATTERN = Pattern.compile("\\$\\{([^/]+?)}"); //${abc}
    private static final Pattern PATH_NOT_REMOVE_VARIABLE_PATTERN = Pattern.compile("#\\{([^/]+?)}");
    private static final Pattern PROTOCOL_PATTERN = Pattern.compile("^[a-zA-Z].+://"); //协议，类似http://
    ...
    private PropertyResolver propertyResolver;  //解析设定的变量（如${abc}），可以包装Properties类型，如System.getProperties()
    private Class<?> clazz; //对应的创建代理类的接口类型,在这里用于解析其注解实现相关功能
    private RequestPreprocessor requestPreprocessor; //
    private ResponseProcessor responseProcessor;  //

    public HttpApiInvoker(PropertyResolver p, Class<?> clazz) {
        propertyResolver = p;
        this.clazz = clazz;
    }
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        ...
        //获取标签中的URL，如果有配置变量则需要替换
        String url = prepareUrl(anno);

        //根据HttpReq中的三个参数url、timeout、method设置HttpRequest对象
    ...
    //将URL中配置的变量${abc}，解析为完整的URL。如果没有协议如http://，那么默认添加HttpApi标签的协议值
    ...
    //将一个Java对象转化为Map<String, Object>键值对，name为对象中的Field名字
        ...
        for (int i = 0, annotationsLength = annotations.length; i < annotationsLength; i++) {
            ...
            for (Annotation ann : annotation) {
                if (ann instanceof Param) {
                    ...
                    //判断args[i]中对象类型是否为InputStream类型或File类型的子类
                    if (isFile(args[i])) {
                        ...
                        //如果参数是原始类型，则param中的value值代表key，该参数值代表value值，并放入map。
                        if (body == null) {
    ...
    //该方法可以替换url中的变量（类似${abc}）,返回替换后的完整URL。具体变量设置可以在System.getProperties()中设置
    private String fillConfigVariables(String url) {
        ...
        while (matcher.find()) {
            ...
            //分隔来指定默认值，如 ${api.url.city:北京} {cityId:1} #{cityId:}
            String defaultValue = null;
            if (key.contains(":")) {
                String[] tmp = key.split(":", 2);
                key = tmp[0];
